...
struct MatchMakingKeyValuePair_t {
    ...
    MatchMakingKeyValuePair_t( const char *pchKey, const char *pchValue ) {
        strncpy( m_szKey, pchKey, sizeof(
                     m_szKey ) ); // this is a public header, use basic c library string funcs only!
...
{
    return ( m_unIP < netadr.m_unIP ) || ( m_unIP == netadr.m_unIP &&
...
inline gameserveritem_t::gameserveritem_t()
{
    ...
    m_nPing = m_nAppID = m_nPlayers = m_nMaxPlayers = m_nBotPlayers = m_ulTimeLastPlayed =
                                          m_nServerVersion = 0;
    m_szGameTags[0] = 0;
}
