...
bool npc_attack_effectiveness::operator>( const npc_attack_effectiveness &rhs ) const
{
    if( !rhs._value ) {
        return true;
    }
...
bool npc_attack_effectiveness::operator>( const int rhs ) const
{
    if( !_value ) {
        return false;
    }
...
npc_attack_effectiveness npc_attack_effectiveness::operator-=( const int rhs )
{
    if( _value ) {
        *_value -= rhs;
    }
...
void npc_attack_melee::use( npc &source, const tripoint &location ) const
{
    ...
    if( !source.is_adjacent( critter, true ) ) {
        add_msg_debug( "%s is attempting a reach attack", source.disp_name() );
        source.reach_attack( location );
    } else {
        add_msg_debug( "%s is attempting a melee attack", source.disp_name() );
...
npc_attack_effectiveness npc_attack_melee::evaluate( const npc &source,
        const Creature *target ) const
{
    npc_attack_effectiveness effectiveness;
    if( !can_use( source ) ) {
    ...
    for( const tripoint &targetable_point : targetable_points( source ) ) {
        if( Creature *critter = g->critter_at( targetable_point ) ) {
            ...
            npc_attack_effectiveness effectiveness_at_point = evaluate_critter( source, target, critter );
...
std::vector<npc_attack_effectiveness> npc_attack_melee::all_evaluations( const npc &source,
        const Creature *target ) const
{
...
npc_attack_effectiveness npc_attack_melee::evaluate_critter( const npc &source,
        const Creature *target, Creature *critter ) const
{
    if( !critter ) {
        return npc_attack_effectiveness{};
    }
    ...
    return npc_attack_effectiveness( std::round( potential ), critter->pos() );
}
...
void npc_attack_gun::use( npc &source, const tripoint &location ) const
{
    ...
    if( !weapon.ammo_sufficient() ) {
        ...
        add_msg_debug( "%s is reloading %s", source.disp_name(), weapon.display_name() );
        return;
    }
    ...
    if( source.aim_per_move( weapon, source.recoil ) > 0 &&
        ...
        add_msg_debug( "%s fires %s", source.disp_name(), weapon.display_name() );
    }
}
...
npc_attack_effectiveness npc_attack_gun::evaluate(
    const npc &source, const Creature *target ) const
{
    ...
    for( const tripoint &targetable_point : targetable_points( source ) ) {
        if( g->critter_at( targetable_point ) ) {
            npc_attack_effectiveness effectiveness_at_point = evaluate_tripoint( source, target,
                    targetable_point );
            effectiveness_at_point -= time_penalty;
            if( effectiveness_at_point > effectiveness ) {
                effectiveness = effectiveness_at_point;
...
std::vector<npc_attack_effectiveness> npc_attack_gun::all_evaluations( const npc &source,
        const Creature *target ) const
{
    std::vector<npc_attack_effectiveness> effectiveness;
    if( !can_use( source ) ) {
...
npc_attack_effectiveness npc_attack_gun::evaluate_tripoint(
    const npc &source, const Creature *target, const tripoint &location ) const
{
    ...
    if( has_obstruction( source.pos(), location ) ) {
        return npc_attack_effectiveness( cata::nullopt, location );
    }
    ...
    return npc_attack_effectiveness( std::round( potential ), location );
}
...
npc_attack_effectiveness npc_attack_activate_item::evaluate(
    const npc &source, const Creature * /*target*/ ) const
{
    npc_attack_effectiveness effectiveness( cata::nullopt, source.pos() );
    if( !can_use( source ) ) {
        return effectiveness;
    }
    ...
    return npc_attack_effectiveness( emergency, source.pos() );
}

std::vector<npc_attack_effectiveness> npc_attack_activate_item::all_evaluations( const npc &source,
        const Creature * /*target*/ ) const
{
    std::vector<npc_attack_effectiveness> effectiveness;
    if( !can_use( source ) ) {
    ...
    // until we have better logic for grenades it's better to keep this as a last resort...
    const int emergency = source.emergency() ? 1 : 0;
    effectiveness.push_back( npc_attack_effectiveness( emergency, source.pos() ) );
    return effectiveness;
}
...
void npc_attack_throw::use( npc &source, const tripoint &location ) const
{
    ...
    if( source.wield( thrown_item ) ) {
        add_msg_debug( "%s throws the %s", source.disp_name(), source.weapon.display_name() );
...
npc_attack_effectiveness npc_attack_throw::evaluate(
    const npc &source, const Creature *target ) const
{
    npc_attack_effectiveness effectiveness( cata::nullopt, source.pos() );
    if( !can_use( source ) ) {
...
std::vector<npc_attack_effectiveness> npc_attack_throw::all_evaluations( const npc &source,
        const Creature *target ) const
{
    std::vector<npc_attack_effectiveness> effectiveness;
    if( !can_use( source ) ) {
...
npc_attack_effectiveness npc_attack_throw::evaluate_tripoint(
    const npc &source, const Creature *target, const tripoint &location ) const
{
    if( has_obstruction( source.pos(), location ) ) {
        return npc_attack_effectiveness( cata::nullopt, location );
